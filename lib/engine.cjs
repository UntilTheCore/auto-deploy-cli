...
const { successLog, errorLog, underlineLog } = require( '../lib/schema.cjs' )
const tiptip = require("./tip.cjs");
...
function deploy( config ) {
  ...
  try {
    ...
    successLog( `\n ${underlineLog( projectName )}项目${underlineLog( name )}部署成功\n` )
    if(process.platform === 'win32') {
      tiptip.successMsgBox()
    }
    process.exit( 0 )
  } catch (err) {
    if(process.platform === 'win32') {
...
function startZip( distPath ) {
  return new Promise( ( resolve, reject ) => {
    ...
    console.log( `打包路径:${underlineLog( distPath )}` )
